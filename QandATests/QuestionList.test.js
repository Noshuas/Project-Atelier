...
test('Renders only a add questions button when no questions are given', () => {
  render(<QuestionList questions={testResults} expanded={false} productName='Test Product' />);
  const input = screen.getByTestId('question-list');
  //console.log(input);
});
...
test('QuestionList snapshot 3', () => {
  const component = renderer.create(
    <QuestionList questions={testResults} expanded={true} productName='Test Product' />
  );
  let tree = component.toJSON();
  expect(tree).toMatchSnapshot();
});